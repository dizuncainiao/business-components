...
微服务子应用通用的概况模块的报表配置框，集成所有动效，只需简单配置即可完成巨量代码功能。
...
| integerMultiple | 允许放置的小图表整数倍（默认为 3） | `number` | —      | 3      |
| smallChartMax   | 小图表的最大数量                   | `number` | —      | 6      |
...
| 事件名     | 说明         | 回调参数               |
| ---------- | ------------ | ---------------------- |
| configured | 点击确认触发 | `function(val: any[])` |
